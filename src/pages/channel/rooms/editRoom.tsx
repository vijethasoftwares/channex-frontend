...
const EditChannelRoom: FC<Props> = () => {
    const { id } = useParams();
    ...
    const handleSubmit = async () => {
        ...
        const obj = {
            ...
            pricePerMonth: parseInt(roomPricePerMonth || "0"),
            maxOccupancy: mo,
            vacancy: mo - guestDetails.length,
            description: roomDescription,
            propertyId: p,
            guestDetails,
            pricePerDay: parseInt(roomPricePerDay || "0"),
            images,
            facilities: rf,
        };
        ...
        try {
            await axios.patch(
                SERVER_URL + "/manager/update-room/" + roomDetails?._id,
                obj,
                {
                    headers: {
    ...
    useEffect(() => {
        const fetchRoomDetails = async () => {
            try {
                const res = await axios.get(SERVER_URL + `/manager/get-room/${id}`, {
                    headers: {
                        Authorization: `Bearer ${user?.token}`,
                    },
                });
                ...
                setRoomNumber(data.room.roomNumber.toString() || "0");
                setProperty(new Set([data.room.propertyId]));
                setRoomCategory(new Set([data.room.roomCategory]));
                setRoomType(new Set([data.room.roomType]));
                setRoomSize(data.room.roomSize.toString() || "0");
                setMaxOccupancy(data.room.maxOccupancy.toString() || "0");
                setIsGuestLiving(data.room.guestDetails.length > 0 ? true : false);
                setGuestDetails(data.room.guestDetails);
                setRoomPricePerMonth(data.room.pricePerMonth.toString() || "0");
                setRoomPricePerDay(data.room.pricePerDay.toString() || "0");
                setImages(data.room.images);
                setRoomFacilities(new Set(data.room.facilities));
                setRoomDescription(data.room.description);
            } catch (error) {
                console.error(error);
            }
        };
        if (id && user?.userId) fetchRoomDetails();
    ...
    useEffect(() => {
        ...
        if (user?.userId) fetchUserProperties();
    }, [user]);

    return (
        <Container>
            <ContainerColumn>
                <Heading>Edit Room</Heading>
            </ContainerColumn>
...
export default EditChannelRoom;
