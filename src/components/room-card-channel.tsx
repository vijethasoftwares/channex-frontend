...
const RoomCardChannel: FC<Props> = ({ data, id, fetchData }) => {
  ...
  const handleDeleteRoom = async (
    ...
    try {
      ...
      const res = await axios.delete(
        `https://rofabs.onrender.com/api/channex/room_type/${data.id}`,
        {
          headers: {
      ...
      <div className="w-full lg:w-2/6 flex flex-col gap-2.5">
        <div className="w-full bg-zinc-50 rounded-md flex">
          <Avatar className="w-full h-48 rounded-xl">
            <AvatarImage
              src={"https://firebasestorage.googleapis.com/v0/b/rofabs-4a4de.appspot.com/o/room_images%2F1709089354163-img1.jpg?alt=media&token=0a19d130-b9aa-4917-aeaf-5ed90c124e30"}
        ...
        <div className="grid grid-cols-2 gap-2.5 p-1">
          <div className="px-3 py-3 rounded-lg bg-zinc-100 flex items-center justify-center text-xs font-semibold">
            <User2 className="mr-1 w-4 h-4" /> Max {data?.default_occupancy} guests
          </div>
          <div className="px-3 py-3 rounded-lg bg-zinc-100 flex items-center justify-center text-xs font-semibold">
      ...
      <div className="absolute right-5 top-5 flex justify-end z-[99]">
        <Dropdown classNames={{ base: "mr-10" }}>
          ...
          <DropdownMenu aria-label="Static Actions">
            <DropdownItem
              onClick={() => navigate("/channel/editRoom/" + data?.id?.toString() || "")}
              key="edit"
            >
...
export default RoomCardChannel;
